...
def bonnetje():
    total = 0
    total += totalbolletjes * 0.95
    print("---------['Papi Gelato']---------")
    print(f'bolletjes   {totalbolletjes} x 1,10   = {round(totalbolletjes * 0.95, 2)} €')
    if soort == 'bakje':
...
def welkesmaken():
    ...
    for i in range(bolletjes):
        try:
            smaak = int(input(f'Welke smaak wilt u voor bol nummer ({i + 1}) 1.Aardbei 2.Chocolade 3.Vanille?\n'))
            if int(smaak) < 0 or int(smaak) > 3:
                print('Sorry dat is geen optie die we aanbieden, we beginnen even overnieuw.')
                welkesmaken()
...
def welkesmakenliters():
    for l in range(liters):
        try:
            smaakliter = int(input(f'Welke smaak wilt u voor liter nummer ({l + 1}) 1.Aardbei 2.Chocolade 3.Vanille?\n'))
            if int(smaakliter) < 0 or int(smaakliter) > 3:
                print('Sorry dat is geen optie die we aanbieden we beginnen weer even overnieuw.')
                welkesmakenliters()
        except ValueError:
            print('Sorry dat is geen optie die we aanbieden, we beginnen weer even overnieuw.')
            welkesmakenliters()
...
def bonnetjezakelijk():
    ...
    print('                        -------- +')
    print(f'                        {totaal}€')
    print(f'btw                     {round(totaal / 100 * 6, 2)}€')
